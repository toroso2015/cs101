...
int main(){
    ...
    time_t now = time(0);
    strftime (YMD,100,"%Y%m%d",localtime(&now));
    strftime (HMS,100,"%H:%M:%S",localtime(&now));
    

    int count[]={1};
    oper_bin=fopen("emp_id.bin","wb+");
    if((fp_bin=fope#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <string.h>
...
f(count), 1 ,fp_bin);
    ...
    time_t curtime;
    time(&curtime);
    int arr_num[5][7],group,flag,temp,userID;
    srand((unsigned) time(NULL));
